    ...
    public void execute() {
        for (int i = 0; i < enemies.size(); i++) {
            ...
            if (newHp == 0) {
                ...
                // Apply ArcaneBlastStatus bonus (+10 attack) for each kill
                ArcaneBlastStatus bonusStatus = new ArcaneBlastStatus(user, 10);
                user.applyStatus(bonusStatus);
                System.out.println(user.getName() + "'s attack increased by 10! (New Attack: " + user.getAttack() + ")");
            }
        }
